...
router.post('/clean-calendar', async (req, res) => {
  try {
    ...
    const calendarListResponse = await googleCalendar.calendar.calendarList.list();
    const calendars = calendarListResponse.data.items;

    logger.info(`📅 ${calendars.length} calendriers trouvés`);
    calendars.forEach(cal => {
      logger.info(`  - "${cal.summary}" (${cal.id})`);
    });

    // Chercher sur 1 an PASSÉ + 1 an FUTUR (pour tout attraper)
    const startDate = new Date();
    startDate.setFullYear(startDate.getFullYear() - 1); // 1 an en arrière

    const endDate = new Date();
    endDate.setFullYear(endDate.getFullYear() + 1); // 1 an en avant

    // Récupérer événements de TOUS les calendriers
    let allEvents = [];
    for (const cal of calendars) {
      try {
        const events = await googleCalendar.getEvents(cal.id, startDate, endDate);
        logger.info(`  📊 "${cal.summary}": ${events.length} événements`);
        allEvents = allEvents.concat(events.map(e => ({ ...e, calendarId: cal.id, calendarName: cal.summary })));
      } catch (error) {
        logger.error(`Erreur récupération ${cal.summary}:`, error.message);
      }
    }

    logger.info(`📅 Total: ${allEvents.length} événements sur ${calendars.length} calendriers`);
    ...
    for (const event of allEvents) {
      ...
      const matches = autoGeneratedKeywords.some(keyword =>
      ...
      if (matches) {
        ...
        // Logger les 5 premiers pour debug
        if (matchCount <= 5) {
          logger.info(`🔍 Match trouvé: "${event.summary}" (description: ${description.substring(0, 50)})`);
        }
      }
    }

    logger.info(`🧹 ${toDelete.length} événements auto-générés trouvés sur ${allEvents.length} total`);

    // Logger quelques titres pour debug
    if (allEvents.length > 0 && toDelete.length === 0) {
      logger.info('📋 Échantillon événements Calendar (pour debug):');
      allEvents.slice(0, 10).forEach(e => {
        logger.info(`  - [${e.calendarName}] "${e.summary}" | description: "${(e.description || 'vide').substring(0, 50)}"`);
      });
    }
    ...
    for (const event of toDelete) {
      try {
        ...
        deleted++;
        logger.info(`🗑️ Supprimé: [${event.calendarName}] "${event.summary}"`);
      } catch (error) {
