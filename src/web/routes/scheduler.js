...
router.post('/clean-calendar', async (req, res) => {
  try {
    ...
    const events = await googleCalendar.getEvents(calendarId, startDate, endDate);

    logger.info(`📅 Récupération événements Calendar: ${events.length} événements sur 2 ans`);
    ...
    if (events.length > 0 && toDelete.length === 0) {
      logger.info('📋 Échantillon événements Calendar (pour debug):');
      events.slice(0, 5).forEach(e => {
        logger.info(`  - "${e.summary}" | description: "${(e.description || 'vide').substring(0, 50)}"`);
      });
    }
